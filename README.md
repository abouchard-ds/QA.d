...
The datasets created with this program can be of a sizable amount. If you take all Canadian and American stocks without exclusion (debentures, warrants, preferred shares, etc.) you could get approximately 2Gb to 5Gb of data. Used to test the new Oracle features on external tables when Oracle Corporation released it (so an ETL from the internet using this script and an external table on the STOCK file). 
